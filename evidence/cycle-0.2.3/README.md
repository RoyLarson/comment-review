...
`feat/0.2.3-cycle-and-record` at `1ad4ba7`. **110 census blocks, 11 holding prose.**
...
as unresolved. `join.txt` here is taken against a worktree pinned at `1ad4ba7`, the tree the
...
pinned-census.json      re-censused at 1ad4ba7 AFTER the raw_lines fix -- what the galley used
records/*.json          the four filled record files, as the roles left them
join.txt                the stage-5 gate, exit 0, against a worktree pinned at 1ad4ba7
