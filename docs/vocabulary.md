...
| **page** | **The file itself** -- everything set on it, in order -- as against the CENSUS, which is that same file as a numbered list of its paragraphs. **A pCST is a page.** ! It is NOT the *proof* and NOT the *galley*, and the three are a sequence the printing register already fixes: a **galley** is text set but *not yet made into pages* (`galley.py` says so in its own first line), a **page** is that text set into one, and a **proof** is a finished page pulled for checking -- which is what `review.md` means by *"the finished page"*. ! An earlier draft of this row defined a page as *the file as a reader meets it*, which is the PROOF: two terms, one meaning, in the entry ruling that the register is the point. |
