...
| **page** | **ONE FILE: its paragraphs in order, among the code they sit with.** A pCST is a page. !! **A PAGE DOES NOT NEST, AND THAT IS WHY THE FLATNESS NEEDED NO APOLOGY.** The pCST is flat because an address is an ORDINAL and an ordinal cannot express containment -- which had been written as a limitation, *pseudo* because a real CST has hierarchy. Roy, 2026-08-19: *"because it is a flat list of paragraphs."* Paragraphs run down a leaf and do not nest, so the shape the addressing forced is the shape the register already had. ! The **census** is the enumeration across EVERY page in scope, not one page as a list -- measured 2026-08-19, one census covering 14 files and 6,678 paragraphs. ! The **proof** is a finished page pulled for checking; the **galley** is text set but not yet made into pages. ! Two earlier drafts of this row were wrong in different directions: one defined a page as *the file as a reader meets it*, which is the PROOF, and one split page from census as *the thing* against *the list of it*, which is no distinction at all once a page is a flat list of paragraphs. |
...
| **galley** | **The proposed text SET AS FILES** -- a trial impression, the text set but *not yet made into pages*, so it can be corrected before anything is committed. `galley.py` writes one per file under `--out`; nothing under `--repo` is touched. ! It is what stage 7a diffs to show the author what will land, and what a round-2 re-review is censused against so a synthesised paragraph has a real address. ! **galley -> page -> proof** is a sequence, and each term names a different moment. |
| **proof** | **a finished page, pulled for checking** -- what stage 8 reads. ! The state, not the thing: the page is the file, and the proof is that page once WRITE has set the approved text on it. |
