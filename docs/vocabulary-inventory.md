...
| ⚠ **original** — **UNRESOLVED** | `ref/residue-check.md:15`, *"Copy the ORIGINAL block whole into a scratch document"* | ONE consistent sense — the block's prose BEFORE this run edited it — at `agents/…-compact.md:14`, `ref/compact.md:50,71`, `ref/residue-check.md:9,15`, `ref/write.md`. It is what the residue check compares against and what COMPACT is handed, and no site says what it is |
...
| ⚠ **template** — **UNRESOLVED** | the MEASURED doc format (`SKILL.md` 1.3, `ref/review.md:15`, `sk-scripts/run_context.py:57`) | the DISPATCH PACKET's own text (`sk-scripts/run_context.py:3,103,179,262`). ⚠ And a THIRD: a C++ language construct at `sk-scripts/census.py:320` |
| ⚠ **gap** — **UNRESOLVED** | a COVERAGE gap — a census index a reviewer never accounted for (`ref/reviewer-brief.md:93`, `sk-scripts/verdicts.py:18`) | *"NOT CHECKED — these are gaps, not passes"*, a file or search that did not complete (`sk-scripts/census.py:1024`, `sk-scripts/referrers.py:159`). ⚠ And a third: a name the module docstring never accounts for (`agents/…-module-context.md:55`) |
| ⚠ **opener** — **UNRESOLVED** | a comment OPENER, the lexer's delimiter (`sk-scripts/census.py:219,354,361`) | a RECORD opener, `--- FINDING` (`sk-scripts/verdicts.py:73`) |
| **walk** | a procedure a reviewer performs — *"Walk the census"* (`ref/reviewer-brief.md:34`), *"walk the module docstring against that list"* | `ast.walk`, a stdlib API (`sk-scripts/census.py:540`). Same shape as `node`: an implementation identifier, not this system's term |
| **annotations** | the census's, settled 2026-08-15 | `from __future__ import annotations`, in every shipped script. A Python future-import, same shape as `walk` |
