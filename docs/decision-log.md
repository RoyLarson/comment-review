...
- **#12.** **An eval case's two hashes are `start` and `end`, not `base` and `fixed`** (Roy,
  ...
  ! **RENUMBERED FROM #10 ON MERGE, 2026-08-24**, by this section's own rule -- both branches
  wrote a `#10` and the entry ACCEPTS BOTH rather than either side winning. The other two keep
  their numbers because they are cited elsewhere; nothing cited this one yet.
