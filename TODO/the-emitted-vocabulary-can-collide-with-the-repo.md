...
term belongs to the repo's sheet. This is settled and is not work.

## Tasks

- [ ] T1 -- * Rule on whether stage 1 must LOOK for collisions, or only record them when a
      reviewer trips over one. ! Looking is cheap and mechanical -- the emitted set is a fixed
      list of at most 46 keys per role and the tree is greppable -- but it will surface words
      like `record`, `page` and `owner` on almost any codebase, and a list of forty near-misses
      is noise. Recommendation: grep the keys against IDENTIFIERS only, not prose, so a hit means
      the repo has a symbol by that name.

- [ ] T2 -- Add a COLLISIONS section to the style sheet, beside the terms of art at
      `SKILL.md:257-264`, and state that a collision is DECLARED rather than resolved. ! Say why:
      neither vocabulary may be renamed to suit the other, so the only useful move is telling the
      reader both meanings are live. Verify: a run over a repo whose code defines a symbol named
      for an emitted term produces a sheet with that term under COLLISIONS.

- [ ] T3 -- Decide whether a declared collision reaches the REVIEWERS, and fix the sentence that
      makes it dangerous. The sheet is in the stage-4 packet, so it would -- but `vocabulary.py:73`
      emits *"where it is not here, it is ordinary English"*, which is wrong for a collided term.
      Verify: the emitted header either names the sheet's COLLISIONS section or the collided term
      carries both meanings where it is defined.

- [x] T4 -- Not a task, and its content is now in the Objective above. "Do not add repo terms to
      `vocabulary.toml`" is a RULE that is true the day it is written and every day after; there
      is no state in which someone ticks it. ! It is also already ENFORCED, which is why it needs
      no box: `check_vocabulary.py`'s drift check refuses a term no role's text uses.
