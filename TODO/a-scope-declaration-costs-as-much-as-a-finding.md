...
Updated:  2026-08-28 — VOCABULARY 2026-08-28: reworded live prose to current terms --
          block -> paragraph at 6 sites (L52, L81, L84, L90, L94, L107 by pre-edit
          numbering) and the self-quoted prohibition's blocks -> paragraphs at 2
          matching sites (Objective and T5, kept identical so "Kept in full" stays
          true). Also backtick-wrapped a bare verdicts.py/record.py mention added in the
          prior triage note, to match this repo's own citation convention.  Left
          unchanged, deliberately: the two "1,174-(prose-)block repo" mentions and the
          derived "roughly a thousand blocks are outside its remit" -- these are a
          measurement's own words from 2026-08-17, counted under the census's old block-
          based addressing before the front/back-matter and leading series were split
          out of it (docs/vocabulary.md:20, CLAUDE.md front/back-matter cluster).
          Renaming them to "paragraph" would assert the same repo counts the same way
          under the current addressing, which has not been checked and may not be true
          -- so the count would no longer be re-derivable from what was actually
          measured. Also left: "block the other roles" (L22, the ordinary English verb,
          quoted from decision-log.md Process #33 in my own prior note) and both `block-
          context` role-name mentions (L62, L83) -- neither is a use of the retired
          paragraph-sense noun.
```
...
the paragraph otherwise leaves the report certified by nobody (`reviewer-brief.md:425-431`).
...
- **That the payload is wasted.** An `outside my role` query with its `SOURCES` proves the role
  read the paragraph. That is the fabricated-`clean` defence, and it is the reason the contract
  exists.
- **The token cost specifically.** `block-context` was measured at ~21 minutes and 491k tokens
  as the heaviest role, and its remit is every claim in every paragraph -- that is a different cause
...
scope owns the paragraph and asks nothing of the author, the other two are work
(`reviewer-brief.md:407-409` says exactly that). A payload that follows the routing is
consistent with it.

**Against it:** `SOURCES` is what proves the paragraph was read, and dropping it on the shape a
...
**Do not solve this by exempting a role from paragraphs outside its remit.** That is the
forced-record problem in reverse, and `module-context` returning `query` rather than `clean` on
a paragraph it never read is deliberate -- it does not certify what it has not seen.
...
- [x] T5 -- NOT A TASK. *"Do not solve this by exempting a role from paragraphs outside its
