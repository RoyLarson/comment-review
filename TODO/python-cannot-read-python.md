...
It is kept in the boxes below as the record of how the ruling was reached.
...
- [ ] T33 -- PROVE_UNCHANGED PROVES A LEXED LANGUAGE, PYTHON INCLUDED. The `stripped`
      ...
      WEAKER GUARANTEE where the docstring now claims the language's own parser:
      Roy, 2026-08-23, it 'can only prove that the code didn't move relative to
      each other'. A projection cannot prove semantic equivalence -- it proves the
