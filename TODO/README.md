...
| --- | --- | :-: | ---: | --- |
| [a-comment-inside-a-line-makes-the-file-unprovable](a-comment-inside-a-line-makes-the-file-unprovable.md) | backend | yes | 2/7 | A comment INSIDE a line makes the whole file unprovable |
...
| [a-notation-carries-its-own-indentation](a-notation-carries-its-own-indentation.md) | backend | yes | 0/3 | MEASURED 2026-08-25 through galley.reset and compositor.set_page over def f(a, b) with an indented comment inside the body. A replacement of '# the sum' lands at COLUMN 0; a replacement of four-spaces-then-'# the sum' lands correctly indented. The galley writes exactly what it is handed, which is Roy's ruling working as intended: it gets the cues-new text or a delete, so that empty lines and spaces are not added to the page. THE CODE IS RIGHT AND THE CONTRACT IS SILENT. desk/notations.py describes an address mapped to a replacement paragraph and never says the text must carry its own leading whitespace, so a desk or an agent that supplies unindented prose gets a comment at column 0 inside a function body. ALL THREE CHAIN CHECKS PASS ON IT, which is why this needs writing down rather than catching. The re-read passes because the cue holds exactly what it was given. prove_unchanged passes because a comment never enters the AST fingerprint. The sha gate passes because the source did not change. A human reading the drafted file is the first thing in the chain that can see it. |
...
| [the-bridge-landed-and-the-rewrite-did-not](the-bridge-landed-and-the-rewrite-did-not.md) | backend | yes | 8/11 | The bridge landed and the rewrite did not |
