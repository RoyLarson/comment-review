...
| [a-block-does-not-say-where-its-text-starts](a-block-does-not-say-where-its-text-starts.md) | session | yes | 2/7 | A block does not say where its text starts, so two things infer it |
...
| [address-collides-across-dotted-paths](address-collides-across-dotted-paths.md) | session | — | 0/3 | **`a/b.py` and `a.b.py` both dot to `a.b.py`, so `a.b.py@a0` names two blocks in two files -- and `--check` passes it.** The dotted form was chosen so mixed languages could not collide; it collides on directory separators instead. |
