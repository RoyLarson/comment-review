...
| --- | --- | :-: | ---: | --- |
| [a-comment-inside-a-line-makes-the-file-unprovable](a-comment-inside-a-line-makes-the-file-unprovable.md) | backend | yes | 2/5 | A comment INSIDE a line makes the whole file unprovable |
...
| [a-scope-declaration-costs-as-much-as-a-finding](a-scope-declaration-costs-as-much-as-a-finding.md) | agents | yes | 1/4 | A scope declaration costs as much as a finding |
...
| [move-and-correct-compose](move-and-correct-compose.md) | agents | — | 4/5 | `move` and `correct` COMPOSE, and the gate calls them a contradiction |
...
| [stage-5-is-the-only-stage-with-no-independent-reader](stage-5-is-the-only-stage-with-no-independent-reader.md) | agents | — | 6/9 | Stage 5 is the only stage whose writer is also its checker |
...
| [the-emitted-vocabulary-can-collide-with-the-repo](the-emitted-vocabulary-can-collide-with-the-repo.md) | agents | yes | 1/4 | The emitted vocabulary can collide with the reviewed repo's own terms |
...
| [docstrings-need-their-own-address-series](docstrings-need-their-own-address-series.md) | backend | — | 5/7 | A docstring needs its own address series, and it names what it documents |
...
| [doc-does-not-fill-its-a](doc-does-not-fill-its-a.md) | backend | — | 4/6 | the a place now exists for Rust/Go/etc but the doc that fills it still sits at b |
| [vocabulary-gate-is-red](vocabulary-gate-is-red.md) | systems | — | 2/5 | check_vocabulary exits 1 on this branch and no test asserts check_drift |
...
| [closing-line-deletes-code](closing-line-deletes-code.md) | backend | — | 5/7 | An edit to a comment whose run closes mid-line DELETES the code after the closer |
| [versioning-at-v1](versioning-at-v1.md) | systems | — | 3/4 | v1.x wants a concrete versioning system on everything that ships |
| [check-passes-a-shared-address](check-passes-a-shared-address.md) | backend | — | 0/3 | addresser --check prints SHARED and exits 0 |
| [a-series-never-fills-outside-python](a-series-never-fills-outside-python.md) | backend | — | 17/18 | Outside Python the `a` place is emitted and never filled |
| [bom-is-read-as-source](bom-is-read-as-source.md) | backend | — | 2/3 | A UTF-8 BOM is censused as a line of code |
| [a-malformed-page-drops-its-records](a-malformed-page-drops-its-records.md) | backend | — | 1/3 | A page entry that is not an object loses every record under it, silently |
...
| [stale-claims-after-the-envelope](stale-claims-after-the-envelope.md) | backend | — | 1/4 | Shipped prose still describes formats and flags this branch deleted |
| [one-stem-four-jobs](one-stem-four-jobs.md) | backend | — | 6/11 | one stem, four jobs -- and the vocabulary sweep cannot see it |
...
| [galley-and-compositor-write-path](galley-and-compositor-write-path.md) | backend | — | 2/5 | The galley can overwrite the file under review, and the compositor reads through a normaliser |
...
| [computed-and-never-read](computed-and-never-read.md) | backend | — | 3/8 | The insert line, the matter branch, the paragraph index and two repeated full-file scans -- each needs more than a comment fix |
...
| [drop-the-column](drop-the-column.md) | backend | — | 6/7 | the column is len(anchor)+1 everywhere; the address system answers what it was added for |
...
| file | owner | roy? | done | what |
| --- | --- | :-: | ---: | --- |
| [compact-can-buy-lines-with-width](compact-can-buy-lines-with-width.md) | agents | — | 2/6 | COMPACT can buy lines with width, and nothing stops it |
...
| [the-bridge-landed-and-the-rewrite-did-not](the-bridge-landed-and-the-rewrite-did-not.md) | backend | yes | 8/10 | The bridge landed and the rewrite did not |
| [corpora-are-all-python](corpora-are-all-python.md) | testing | — | 3/6 | The corpora are nine Python projects, so every per-language rule is measured on Python and C alone |
| [7a-can-prove-the-change-by-applying-it-to-a-copy](7a-can-prove-the-change-by-applying-it-to-a-copy.md) | agents | yes | 3/5 | 7a can PROVE the change by applying it to a copy and diffing |
...
| [a-block-is-a-paragraph-on-a-page](a-block-is-a-paragraph-on-a-page.md) | backend | — | 3/5 | everything read by a human or an agent says paragraph; 706 internal identifiers remain |
| [b-addresser-uninitialised](b-addresser-uninitialised.md) | backend | yes | 13/17 | The b addresser is never initialised at the module trigger, and computes its cue from line numbers |
...
| [ownership-is-read-first-but-nothing-makes-it-so](ownership-is-read-first-but-nothing-makes-it-so.md) | agents | — | 4/10 | `ownership-context` is read FIRST, and nothing in the run makes that true |
...
| [lexer-and-language-findings](lexer-and-language-findings.md) | backend | — | 3/23 | Ten findings in lexer.py and language.py, from three review rounds |
...
| [correct-against-patch-is-a-conflict-and-is-not-flagged](correct-against-patch-is-a-conflict-and-is-not-flagged.md) | agents | — | 0/3 | `correct` against `patch` is a conflict, and the gate does not flag it |
...
| [front-half-undetermined](front-half-undetermined.md) | testing | yes | 3/7 | The census to findings to verdicts path has never been determined against a backend that works |
...
| [stage-4b-is-undefined](stage-4b-is-undefined.md) | agents | yes | 0/4 | the 4a/4c split promises a resolved placement that nothing produces |
...
| [a-closing-quote-with-a-comment](a-closing-quote-with-a-comment.md) | backend | — | 0/1 | the closing quote and the comment beside it are both stored, so the file gains a line |
...
| [docstrings-that-contradict-themselves](docstrings-that-contradict-themselves.md) | backend | — | 2/6 | each states a rule and then denies it, in the same file |
| [isolate-the-codes-contribution](isolate-the-codes-contribution.md) | testing | — | 4/6 | Two graded arms, orchestration held constant; the rewording is the confound |
---
