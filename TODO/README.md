...
| --- | --- | ---: | --- |
| [the-author-approves-blocks-and-never-sees-the-page](the-author-approves-blocks-and-never-sees-the-page.md) | session · Roy (2 rulings) | 1/6 | ⭐ **Pipeline, not vocabulary.** 7a shows the author a per-block LIST; stage 8 is the only pass that reads the PAGE, and it runs AFTER 7b has written to disk. So every defect `review.md` exists to catch — a block that is no longer a proposition, two runs merged across a blank line, the same sentence in two places — is found after approval and after the write. Roy wants a whole-document read BEFORE the person sees it, and floated a temporary branch with the diff so they can accept it in git's own tools. Stage 8 then becomes a verification with two outcomes: good, or raise to human as a new review. ⚠ Already done: the 7b paragraph claiming *"this pass cuts, and it can cut a lot"* is deleted — self-contradicting since the import |
