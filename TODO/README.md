...
| [master-proof-and-edit-copy](master-proof-and-edit-copy.md) | backend | — | 0/5 | `seed` returns a flat mark list and nothing holds the several copies one stage produces; `docs/vocabulary.md` lists `master proof` as unnamed |
| [topology-is-a-source-edit](topology-is-a-source-edit.md) | backend | — | 0/5 | `desk/stages.py`'s `STAGES` is a two-row literal, so a run cannot say which roles run when, what each reads, or how a role fans out |
