...
| [a-revise-answer-has-no-artifact](a-revise-answer-has-no-artifact.md) | backend | yes | 17/31 | A role can be asked to revise and has nothing to answer ON |
...
| [no-command-for-the-middle](no-command-for-the-middle.md) | backend | -- | 8/14 | `gather`, `places`, `reconcile` and `docket_from` have no CLI face, so nothing turns checked marks into the docket `proof --docket` requires. Measured 2026-08-29 on a real run: every other step of the chain is a command; this one had to be driven from a hand-written script |
