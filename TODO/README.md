...
| [a-block-does-not-say-where-its-text-starts](a-block-does-not-say-where-its-text-starts.md) | session | yes | 4/9 | A block does not say where its text starts, so two things infer it |
...
| [anchor-was-empty-on-98-percent](anchor-was-empty-on-98-percent.md) | session | yes | 2/4 | the anchor was wired into the record and never populated; fixed, with the gates that would have caught it |
