...
| [a-block-does-not-say-where-its-text-starts](a-block-does-not-say-where-its-text-starts.md) | session | yes | 4/8 | A block does not say where its text starts, so two things infer it |
| [a-comment-inside-a-line-makes-the-file-unprovable](a-comment-inside-a-line-makes-the-file-unprovable.md) | session * Roy (* 1 ruling, MADE) | yes | 1/5 | A comment INSIDE a line makes the whole file unprovable |
...
| [correcting-one-copy-strands-the-reference-copy](correcting-one-copy-strands-the-reference-copy.md) | Roy | yes | 0/6 | Correcting one copy strands the copy in a REFERENCE ONLY file |
| [move-and-correct-compose](move-and-correct-compose.md) | session * Roy (1 ruling, made) | — | 4/5 | `move` and `correct` COMPOSE, and the gate calls them a contradiction |
| [nothing-checks-that-four-reviewers-were-launched](nothing-checks-that-four-reviewers-were-launched.md) | session * Roy (* 1 ruling) | yes | 1/5 | Nothing checks that four reviewers were LAUNCHED |
| [only-census-got-out-and-the-skill-instructs-a-redirect](only-census-got-out-and-the-skill-instructs-a-redirect.md) | session | — | 0/4 | Only `census.py` got `--out`, and `SKILL.md` instructs the redirect it forbids |
| [ownership-is-read-first-but-nothing-makes-it-so](ownership-is-read-first-but-nothing-makes-it-so.md) | session (serialisation and 4b ruled 2026-08-17; the rest is build) | yes | 3/10 | `ownership-context` is read FIRST, and nothing in the run makes that true |
...
| [two-live-runs-proposed-fifteen-changes](two-live-runs-proposed-fifteen-changes.md) | session * Roy (* 3 rulings) | yes | 5/16 | Two live runs proposed fifteen changes |
...
| [docstrings-need-their-own-address-series](docstrings-need-their-own-address-series.md) | session * Roy (* 1 ruling -- the numbering) | yes | 5/7 | A docstring needs its own address series, and it names what it documents |
...
| [shipped-prose-lags-the-rulings](shipped-prose-lags-the-rulings.md) | session | — | 0/6 | **The address rulings landed in the code and in `docs/`; the files agents actually read were not brought along.** Four agents found the same shape independently: ~25 statements now false, several contradicting themselves inside one docstring. |
| [deprecated-reader-cannot-replay](deprecated-reader-cannot-replay.md) | session | — | 0/3 | **`--convert` drops 100% of a held 0.2.x report and exits 0.** Replaying held reports is how 0.2.1 and 0.2.2 were validated cheaply -- five joins over one set of reports, ~1.6M tokens of review reused -- and that property is the stated reason the deprecated parser was kept. |
| [address-collides-across-dotted-paths](address-collides-across-dotted-paths.md) | session | yes | 0/3 | **`a/b.py` and `a.b.py` both dot to `a.b.py`, so `a.b.py@a0` names two blocks in two files -- and `--check` passes it.** The dotted form was chosen so mixed languages could not collide; it collides on directory separators instead. |
...
| [c-series-admitted-not-writable](c-series-admitted-not-writable.md) | session | yes | 0/5 | **The join says `Every finding is admissible` on an `add` at `@c0`; the galley then refuses the splice AND discards every other edit in that file.** The `c` series exists so a trailing comment can be placed -- Roy: *"without the cs being there you can't specify that the comment belongs at the end of the code line"* -- and nothing can write one. |
