...
| [topology-is-a-source-edit](topology-is-a-source-edit.md) | backend | — | 4/5 | `desk/stages.py`'s `STAGES` is a two-row literal, so a run cannot say which roles run when, what each reads, or how a role fans out |
