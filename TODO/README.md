...
| [doc-does-not-fill-its-a](doc-does-not-fill-its-a.md) | backend | — | 4/8 | the a place now exists for Rust/Go/etc but the doc that fills it still sits at b |
...
| [a-page-carries-no-identity](a-page-carries-no-identity.md) | backend | — | 4/4 | A page carries no identity, so staleness is checked by re-parsing and comparing |
...
| [an-alteration-carries-its-own-indentation](an-alteration-carries-its-own-indentation.md) | backend | -- | 0/3 | An alteration carries its own indentation and nothing says so |
...
| [shape-is-three-words](shape-is-three-words.md) | backend | -- | 0/6 | `shape` carries three senses, only one reaches an agent as a field name; and the axis it names is on `query` alone while `clean` has a recorded, unfilled need for the same thing |
...
| [brief-says-prose-is-withheld](brief-says-prose-is-withheld.md) | agents | -- | 0/3 | reviewer-brief.md:113 says the record withholds the prose so a role cannot rule without reading the code; flows/marks.py:81 puts raw_text on every mark and collator.py makes it load-bearing |
...
| [no-command-for-the-middle](no-command-for-the-middle.md) | backend | yes | 3/10 | `gather`, `places`, `reconcile` and `docket_from` have no CLI face, so nothing turns checked marks into the docket `proof --docket` requires. Measured 2026-08-29 on a real run: every other step of the chain is a command; this one had to be driven from a hand-written script |
...
| [containers-and-verification-are-unwired](containers-and-verification-are-unwired.md) | backend | yes | 11/39 | The containers and the source-verification half are wired to nothing |
...
| [collate-command-defects](collate-command-defects.md) | backend | -- | 1/18 | Sixteen defects in `commands/collate.py`, measured by running it |
...
| [no-mark-for-let-it-stand](no-mark-for-let-it-stand.md) | agents | yes | 1/16 | There is no mark for LET IT STAND -- a declined proposal is not recorded, so the next run proposes it again |
| [front-half-undetermined](front-half-undetermined.md) | testing | yes | 3/11 | The census to findings to verdicts path has never been determined against a backend that works |
...
| [strip-and-fill-the-markers](strip-and-fill-the-markers.md) | backend | -- | 0/8 | A role is handed the paragraph with its `#`/`"""` markers and indentation, and must reproduce them exactly in `change`. Measured 2026-08-29: three consecutive attempts by an agent with full context broke the file, and each failure read as a defect in the setter |
| [task-agent-vocabulary-home](task-agent-vocabulary-home.md) | agents | yes | 0/4 | The task agent has no vocabulary home and is never told the command set |
