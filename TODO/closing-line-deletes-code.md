...
TRIAGED:  2026-08-23 — 2026-08-23. Five records: the measurement, the half-cut cause,
          that prove_unchanged DOES catch it but only after 7b has written to disk, the
          three unchanged options, and the sequencing note that this closes with the
          galley because the composition step sees the whole file. ! ONE LIVE ITEM: the
          RE-RULING in task 4. Roy accepted this residue on 2026-08-20 when it was
          described as a moved interval boundary; the cost is DELETED CODE. The
          measurement that justified accepting still stands -- 0 occurrences in 180,821
          lines -- so the question is whether that frequency still buys the acceptance
          now the consequence is named correctly.
```
...
- [x] !! MEASURED 2026-08-21. On `int a = 1;\n/* note\n   more */ int x = 5;\nint
      ...
      `int x = 5;` is GONE.
- [x] ! THE CAUSE IS A HALF-CUT. The 2026-08-20 fix trims the paragraph's TEXT at
...
- [x] ! `prove_unchanged._delimiter_shares_the_line` DOES catch it -- the file
...
- [x] ! THE OPTIONS ARE UNCHANGED: a field for where the paragraph's text ENDS on
...
- [x] ! CLOSE THIS WITH THE GALLEY, not before. The composition step sees the
