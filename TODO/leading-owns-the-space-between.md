...
- [x] * RULED 2026-08-21 -- A FIFTH SERIES, `leading`, OWNS THE SPACE BETWEEN
...
- [x] !! THE TRADE WORD IS `leading` -- the strips of lead a compositor puts
...
- [x] THE RULE, in Roy's words: *"it covers all empty space between two different
...
- [x] !! IT IS NOT CITABLE. Roy: *"it does not need to be citable at all. There is
      ...
      reviewer is handed.
- [x] * RULED -- THE SERIES LETTER IS `d`. Roy, 2026-08-21: *"and d works."* ! `l`
...
- [ ] ! SO IT DOES CARRY A CUE, which the letter settles. The alternative
      ...
      address.
- [x] !! WHY IT IS NEEDED, MEASURED 2026-08-21. A `b` owns the blanks on BOTH
...
- [x] ! IT IS THE ORDINARY SHAPE OF A PYTHON FILE, not an exotic one: a licence
      ...
      behind that shape.
- [x] MEASURED over 3,082 files: 3,049 byte-identical, 3,075 identical IGNORING
      ...
      defect.
- [x] ! THE COLLAPSE-AND-REGENERATE ALTERNATIVE WAS MEASURED AND IS WHY THIS WON.
...
- [ ] !! LEADING IS STILL HANDED TO A REVIEWER, so task 6's claim is half false.
      It is correctly out of `Page.prose` -- `d` names no place, so nothing seeds
      a record on it -- but `census.py --filtered`, which `census.py:510` calls
      'the command SKILL.md hands a reviewer', emits the leading rows anyway: 4 on
      `tests/fixtures/sample.py`, the same count as an unfiltered run. Verify: a
      filtered listing shows 0 leading rows, and an unfiltered one still shows
      them.
