...
Raised:   2026-08-21 (Roy, 2026-08-21, ruling on a `b` whose lines straddle an `a`: 'I
          like the leading solution even though it added another foliation and the
          anchors are empty')
```
...
- [ ] * RULED 2026-08-21 -- A FIFTH SERIES, `leading`, OWNS THE SPACE BETWEEN
      ...
      by name -- another foliation, and anchors that are empty.
...
- [ ] * RULED -- THE SERIES LETTER IS `d`. Roy, 2026-08-21: *"and d works."* ! `l`
      ...
      `m.py@l0` reads as `@10`, and the whole scheme rests on a folio being
      unmistakable. `d` is free, unambiguous beside `a`, `b`, `c` and `f`, and
      cannot be misread as a digit.
- [ ] ! SO IT DOES CARRY A FOLIO, which the letter settles. The alternative
      considered was no address at all -- an address is what makes a thing
      citable, and this is not -- but a folio keeps `Foliation.reading` a plain
      list of folios and keeps the compositor a lookup. ! WHAT IT IS NOT is SEEDED
...
- [ ] !! WHY IT IS NEEDED, MEASURED 2026-08-21. A `b` owns the blanks on BOTH
      sides of an `a` -- ruled, and the covering is exact -- but a folio is ONE
