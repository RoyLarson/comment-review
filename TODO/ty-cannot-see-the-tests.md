...
- [ ] T1 | Implement a `[tool.ty]` section in `pyproject.toml` naming both
      ...
      `CLAUDE.md` passes a path.
- [ ] T2 | Update `tests/gates/test_build.py` and
      `tests/gates/test_dead_sweep.py` so their `scripts/` imports resolve for
      the checker. Verify: `ty` reports zero `unresolved-import`, and both tests
      still run.
- [ ] T3 | Delete every unguarded `.group(` on an unchecked match in `tests/`,
...
- [ ] T4 | Implement the corrections for the ten `invalid-argument-type` and two
...
- [ ] T5 | Update `CLAUDE.md`'s command block to the bare invocation and state a
      DATED green baseline for `ty` over both trees. Verify: the block names no
      path, and the baseline sentence carries the date it was measured.
