...
- [x] * Rule whether the ladder goes entirely or keeps one rung. `proof` is the odd one: it
...
- [x] Remove `LEVELS` from `sk-scripts/run_context.py:96` and its validation at `:246-247`, and
...
- [x] Remove `LEVELS` from `sk-scripts/verdicts.py:65-70`, the `allowed()` membership test at
...
- [x] Remove the ladder from `SKILL.md:183-208` -- the table, *"`ownership-context` runs at every
...
- [x] Remove it from `ref/reviewer-brief.md:5` (*"a restricted `level` runs fewer"*) and
...
- [x] Check what else keyed on it. `move`'s availability, `query`'s use as the fallback when a
