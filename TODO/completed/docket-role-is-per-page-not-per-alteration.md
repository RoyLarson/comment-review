...
- [x] T1 | RULED -- role is ONE PER PAGE: which role set a place is answered by which copy was pulled from. Process 81 names what the docket requires and what _set_by reads | 8ed4688 | Rule
      whether an alteration carries its own role. Verify: the ruling is recorded
      in docs/decision-log.md, naming what docket.read requires and what
      flows.revise.pull._set_by reads
- [-] T2 | SUPERSEDED -- the ruling is that an alteration does NOT carry its own role; the page does | 8ed4688 | Carry
      the role per alteration, if that is the ruling. Verify: a docket built
...
- [-] T3 | SUPERSEDED -- docket_from left desk/ at P55, and there is no omission to retire: a page carries the role of the copy it was pulled from | 8ed4688 | Retire
      docket_from's omission once the format carries it. Verify: no page a
