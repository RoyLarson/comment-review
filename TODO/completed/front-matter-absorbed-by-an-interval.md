...
- [x] T1 -- !! VERIFIED. On `#!/usr/bin/env python` / `# Copyright 2024` / `"""Module
...
- [x] T2 -- !! SO AN `add` ON `b1` REPLACES THE LICENCE HEADER, SILENTLY, rc=0.
...
- [x] T3 -- CAUSE, `page.py` `fill_the_gaps`: `here` collects every paragraph whose
      ...
      other.
- [x] T4 -- AND THE SAME LINE DOUBLE-OWNS A DOCSTRING. `here[-1].end = free[-1]` spans
...
- [x] T5 -- !! INTRODUCED BY 4d576d3, 2026-08-20 -- this session's own work, hours after
...
- [x] T6 -- * RULING MADE, NOT OWED, so this carries no work. It asked whether
      `fill_the_gaps` must exclude a paragraph whose cue is not the gap's own, or
      whether `b0` must stop being in the `b` series. **Roy ruled the second on
      2026-08-20** -- front matter gets its own foliation -- and the code did it:
      VERIFIED 2026-08-23, `page.py:914-922` builds `exact` from `series != GAP`,
      naming no series and carving no exception, with the reason stated at
      `page.py:901-906`. A ruling already made is not a task.
