...
- [x] !! MEASURED 2026-08-21 on a four-line C file. `/* Copyright ... */` then
...
- [x] !! IT REFUSES CORRECT WORK. `entry_for` returns the FIRST, so a `correct` on
...
- [x] ! IT FALSIFIES TWO SHIPPED CLAIMS: `record.entry_for`'s docstring says 'an
...
- [x] * RULING WANTED: what makes them distinct. The `a` series solved this for
      ...
      close this one.
- [x] ! THE FRONT-MATTER RULING TAKES ONE OF THE TWO MEASURED CASES OFF THIS TODO.
...
- [x] !! IT NOW BLOCKS THE COMPOSITOR, WHICH IS A HARDER CONSEQUENCE THAN THE ONE
...
- [x] ! SO THE IDENTITY IS NOW A MECHANICAL DETECTOR FOR THIS.
...
- [x] ! `--edits` HAS THE SAME DEPENDENCY. `galley.py` is keyed by address, so a
