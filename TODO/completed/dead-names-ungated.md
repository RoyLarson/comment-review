...
- [x] !! RUFF CANNOT SEE THEM. It flags an unused IMPORT (F401) and an unused
...
- [x] The sweep that found them ran off the CodeGraph index: for each shipped
...
- [x] ! IT MUST COUNT SAME-FILE CALLERS. The first pass discounted them and
      reported 32 false positives -- a function reached only by its own module's
      `main` IS live, because the CLI is a consumer.
- [x] ! AND IT MUST GREP BEFORE BELIEVING. CodeGraph does not resolve a dataclass
      ...
      off.
- [x] * RULING WANTED: gate or input? `check_vocabulary.py` is a gate and
