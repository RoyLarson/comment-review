...
- [ ] ⭐ Rule on whether a CONTINUATION line is part of the trailing comment or its own block.
      ...
      **(b)** a comment on the line IMMEDIATELY after a trailing comment, with no code and no
      blank line between, continues that run;
      **(c)** keep the split and STAMP it, so a reviewer is told the block is a continuation and
      does not file `correct` against a manufactured mid-clause.
