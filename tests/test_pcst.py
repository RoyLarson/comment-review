...
import ast
import unittest  # noqa: I001  -- path shim below must import before pcst

import pcst
from _paths import SCRIPTS  # noqa: F401

SIBLINGS = {p.stem for p in SCRIPTS.glob("*.py")} - {"pcst"}


class TestItIsALeaf(unittest.TestCase):
    def test_pcst_imports_no_sibling(self):
        tree = ast.parse((SCRIPTS / "pcst.py").read_text(encoding="utf-8"))
        imported = set()
    ...
    def test_every_module_that_reads_a_block_can_import_one(self):
        ...
        for name in ("addresser", "record", "census"):
            with self.subTest(module=name):
                ...
                self.assertIn("from pcst import", text)
...
class TestTheTwoKindSetsAreNotInterchangeable(unittest.TestCase):
    ...
    def test_a_trailing_comment_occupies_nothing_but_holds_prose(self):
        self.assertIn("trailing-comment", pcst.OCCUPIES_NOTHING)
        self.assertNotIn("trailing-comment", pcst.HOLDS_NO_PROSE)

    def test_the_empty_kinds_are_in_both(self):
        for kind in ("interval", "undocumented"):
            with self.subTest(kind=kind):
                self.assertIn(kind, pcst.OCCUPIES_NOTHING)
                self.assertIn(kind, pcst.HOLDS_NO_PROSE)
