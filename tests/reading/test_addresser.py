"""Three addressers, each holding its own counter AND the places it emitted.

!! WHAT A ADDRESSER IS. Roy, 2026-08-19: *"the addresser gets an anchor and emits
an address and should add the address and the anchor to an internal list or
dict"*, and *"a b and c all get addressers."* None was one before this: `a` read
...
addresser doesn't fire ever on things that can't get a doc string"* -- so its
...
class TestAAddresserHoldsItsOwnSteps(unittest.TestCase):
    ...
    def test_a_SKIPPED_trigger_takes_no_number(self):
        ...
        Roy, 2026-08-20: *"the cues own their own rules on what is skipped
...
class TestEveryPlaceRecordsTheTriggerItFiredAt(unittest.TestCase):
    ...
    ! Driven by `cue`, because the claim is about the WALK. A hand-built
    `Addresser` handed two numbers can only show that `emit` assigns them.
    """

    def setUp(self):
        self.cues = addresser.cue(EDGE_CODE, EDGE_DOCUMENTABLE)
        self.walk = addresser.triggers(list(EDGE_CODE))
    ...
    def test_the_walk_OPENS_on_the_module_and_CLOSES_on_eof(self):
        ...
        for name, code in (
            ...
            ("no code at all", {}),
            ("one line", {1: "x = 1"}),
            ("code that SPELLS a sentinel", {1: addresser.EOF, 2: addresser.MODULE}),
        ):
            with self.subTest(shape=name):
                walk = addresser.triggers(list(code))
                self.assertEqual(walk[0], addresser.MODULE)
                self.assertEqual(walk[-1], addresser.EOF)
                self.assertEqual(walk.count(addresser.MODULE), 1)
                self.assertEqual(walk.count(addresser.EOF), 1)
    ...
    def test_indexing_the_walk_with_what_a_place_REPORTS_gives_its_trigger(self):
        """!! THE INVARIANT: `triggers()[anchor_num(cue)]` is the trigger that
        ...
        satisfied by re-deriving the number from the cue.
        ...
        for cue in sorted(places):
            at = self.cues.anchor_num(cue)
            with self.subTest(cue=cue):
                trigger = self.walk[at]
                if isinstance(trigger, int):
                    self.assertEqual(EDGE_CODE[trigger], self.cues.anchor_of(cue))
                else:
                    self.assertIn(trigger, (addresser.MODULE, addresser.EOF))
...
class TestTheWalkOverRoysEdgeCase(unittest.TestCase):
    ...
    def test_every_place_carries_the_line_of_code_it_is_attached_to(self):
        ...
        self.assertEqual(self.places["f0"], addresser.MODULE)
        ...
        self.assertEqual(self.places["b7"], addresser.EOF)
        self.assertEqual(self.places["c0"], "N = 0")

    def test_no_cue_is_emitted_twice(self):
        self.assertEqual(len(self.places), len(set(self.places)))


class TestReadingTheCuesBack(unittest.TestCase):
    """!! WHICH ADDRESS DOES THIS LINE BELONG TO RIGHT NOW.

    Roy, 2026-08-19, on why the cues owns both directions: it *"helps the
    ...
    def test_a_gap_answers_with_the_b_the_walk_emitted_there(self):
        ...
        self.assertEqual(self.cues.above(4), "b2")
    ...
    def test_above_the_first_line_of_code_is_b0(self):
        ...
        self.assertEqual(self.cues.above(1), "b0")

    def test_past_the_last_line_is_the_closing_gap(self):
        self.assertEqual(self.cues.above(99), "b7")

    def test_a_line_of_code_answers_with_its_own_c(self):
        self.assertEqual(self.cues.beside(3), "c1")

    def test_a_line_holding_no_code_has_no_c(self):
        self.assertEqual(self.cues.beside(1), "")

    def test_a_declaration_answers_by_its_ORDINAL_not_its_line(self):
        self.assertEqual(self.cues.documents(0), "a0")
        self.assertEqual(self.cues.documents(1), "a1")
        self.assertEqual(self.cues.documents(2), "a2")

    def test_a_declaration_the_file_does_not_have_answers_nothing(self):
        self.assertEqual(self.cues.documents(9), "")
...
class TestAFifthSeriesWouldNotNeedFindingFourTimes(unittest.TestCase):
    ...
    !! THE FOURTH COST TWO BUGS AND A DEAD CLI FLAG. `cue` merged three
    addressers' places and not the fourth, so `f0` had no anchor and no
    ...
    def test_every_series_constant_is_in_the_list(self):
        for name in ("DECLARED", "GAP", "ON", "COVERS"):
            with self.subTest(series=name):
                self.assertIn(getattr(addresser, name), addresser.SERIES)

    def test_no_two_series_share_a_letter(self):
        self.assertEqual(len(set(addresser.SERIES)), len(addresser.SERIES))
    ...
    def test_every_place_the_walk_emits_carries_an_anchor(self):
        ...
        got = addresser.cue({1: "N = 0", 2: "def f():"}, {1: (2, 1, "c")})
        self.assertTrue(got.places)
        for cue, anchor in got.places.items():
            with self.subTest(cue=cue):
                self.assertTrue(anchor, f"{cue} carries no anchor")
        # ! THE SECOND HALF OF THIS IS NOW STRUCTURAL. It read `for cue in
        # got.bounds: assertIn(cue, got.places)` -- a place could sit in one
    ...
    def test_every_place_answers_with_the_line_its_anchor_sits_on(self):
        ...
        got = addresser.cue({2: "N = 0", 3: "def f():"}, {1: (4, 2, "b")})
    ...
    def test_a_SENTINEL_answers_None_because_it_sits_on_no_line(self):
        ...
        got = addresser.cue({2: "N = 0"}, {})
        for cue in ("a0", "f0", "b1", "f1"):
            with self.subTest(cue=cue):
                self.assertIsNone(got.anchor_line(cue))
