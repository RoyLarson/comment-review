...
## The subject is the JOINED PARAGRAPH, never the finding
...
The paragraph change is already done, sending it right back doesn't help."*

So a role is sent the paragraph **as stage 5 or stage 6 left it**, holding every role's edits at
once, and answers three questions about it:

1. Did my edit SURVIVE into this paragraph?
...
!! **SO A PARAGRAPH GOES BACK TO THE ROLES THAT FILED ON IT, AND TO NO OTHERS.** Ruled 2026-08-17.
...
joined paragraph asks a different question in the same envelope, and it would come back with what a
...
broke, and stage 8 reads the finished page against itself -- but between the two, a paragraph that
...
- **The joined paragraph**, in full, as it now reads.
...
- **`HOLD`** -- the paragraph carries what I filed, and the other edits do not break it. Nothing
  further is owed.
...
nobody has to guess it: stage 4 is the first read and is not a round here. A paragraph may go back
...
tie-break"* forbids stage 5 preferring one role over another INSTEAD OF sending the paragraph back.
...
! **Say in the report which paragraphs were ruled this way, and on what.** A paragraph stage 5 settled
...
Paragraphs sent, per slot; `HOLD` and `REVISE` counts per role; every paragraph still split after one
round, named, with both answers. ! A paragraph that went back and returned nothing is a finding, not
a silence.
